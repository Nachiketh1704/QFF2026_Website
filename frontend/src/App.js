...
const schedule = [
  { day: "Day 1", title: "Technical Talks", date: "25th November 2026", items: [["TBA", "Agenda will be announced closer to the event.", ""]] },
  { day: "Day 2", title: "Workshop", date: "26th November 2026", items: [["TBA", "Agenda will be announced closer to the event.", ""]] },
  { day: "Day 3", title: "Quantum Odyssey", date: "27th November 2026", items: [["TBA", "Agenda will be announced closer to the event.", ""]] },
];
  ...
  return <div className="site-shell">
    ...
    <main id="top">
      <section className="hero" data-testid="hero-section">
        ...
        <div className="hero-stamp" data-testid="hero-campus-note">IN-PERSON ONLY<br /><span>AMRITAPURI CAMPUS</span></div>
      </section>
      ...
      <section className="intro section" id="event" data-testid="about-section"><div className="section-kicker">01 <span>ABOUT THE FEST</span></div><div className="intro-layout"><h2 data-testid="about-heading">Student-led<br /><span>quantum festival</span><br />powered by Qiskit.</h2><div className="intro-text"><p>Organized by students at Amrita Vishwa Vidyapeetham, Amritapuri Campus. Join workshops and speaker sessions to explore quantum computing with Qiskit through interactive learning activities.</p><div className="tracks"><div data-testid="track-learn"><b>01</b><strong>Technical Talks</strong><span>Hybrid lecture series featuring IBM and hardware domain experts covering quantum architecture, frontier research, and career pathways.</span></div><div data-testid="track-build"><b>02</b><strong>Workshop</strong><span>Hands-on Qiskit workshop with IBM-certified speakers. Build quantum circuits, run on simulators and real hardware.</span></div><div data-testid="track-compete"><b>03</b><strong>Quantum Odyssey</strong><span>Team-based hackathon-style competition with problem sets ranging from beginner to advanced, with prizes for top performers.</span></div></div><div className="partner-row" data-testid="partner-row"><small>In partnership with</small><div className="partner-marks"><img src="/assets/IBM_Quantum/Raster/RGB/IBM_Quantum_logotype_pos_RGB.png" alt="IBM Quantum" data-testid="ibm-quantum-logo" /><img src="/assets/Pictogram/SVG/qiskit_purple-60.svg" alt="Qiskit" data-testid="partner-qiskit-mark" /></div></div></div></div></section>

      <section className="schedule section" id="schedule" data-testid="schedule-section"><div className="section-kicker">02 <span>THE PROGRAM</span></div><div className="section-heading-row"><h2 data-testid="schedule-heading">Event<br /><span>schedule.</span></h2><p>Draft agenda — final timings and rooms will be announced closer to the event. This is an in-person event held at Amrita Vishwa Vidyapeetham, Amritapuri Campus. Attendance requires physical presence at the venue.</p></div><div className="day-grid">{schedule.map((day) => <article className="day-card" key={day.day} data-testid={`schedule-${day.day.toLowerCase().replace(' ', '-')}`}><div className="day-top"><div><span>{day.day}</span><h3>{day.title}</h3></div><time>{day.date}</time></div><div className="schedule-list">{day.items.map(([time, title, speaker], i) => <div className="schedule-row" key={i} data-testid={`schedule-item-${day.day}-${i}`}><time>{time}</time><div><strong>{title}</strong>{speaker && <small>{speaker}</small>}</div></div>)}</div></article>)}</div></section>
